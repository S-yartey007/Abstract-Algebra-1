...
If $H$ is a normal subgroup, we have now constructed in great detail a group $G/H$ and a surjective homomorphism $\pi: G \to G/H$. What is the kernel of $\pi$? The identity of $G/H$ is the coset $H$, which is $H$ itself. [81, 82] So
...
Every group homomorphism $\varphi: G \to G'$ may be decomposed as follows: [96, 97, 102]
$$G \xrightarrow{proj} G/\ker \varphi \xrightarrow{\cong} \text{im } \varphi \xrightarrow{incl} G' \text{ [100, 103, 104, 106, 108]}$$
where the isomorphism in the middle is the homomorphism induced by $\varphi$ (as in Theorem 4.12). [109]

Theorem 5.1 makes one aware that to any group homomorphism $\varphi: G \to G'$, it can immediately be seen that $\frac{G}{\ker \varphi}$ identifies with a subgroup of $G'$. [110, 111, 116]

\subsection*{Corollary 5.2}
Suppose $\varphi: G \to G'$ is a surjective group homomorphism.
$$G' \cong \frac{G}{\ker \varphi} \text{ [120]}$$

\textbf{Proof} \\
From Theorem 5.1, $\text{im } \varphi = G'$ since $\varphi$ is surjective and so the result holds. [122]
...
$$\frac{D_6}{C_3} \cong C_2. \text{ [127]}$$
We can check this by hand, noting that there is a surjective group hom. $D_6 \to C_2$ whose kernel is $C_3$: [129, 130] send an element $\sigma \in D_6$ to the identity in $C_2$ if it does not flip the triangle (this is exactly the case when $\sigma \in C_3$), and send the element to the other element in $C_2$ if it does. [130] Then by Corollary 5.2 we obtain the desired results. [131]
...
Note: As an exercise show that if $H \subseteq K$ are subgroups of a group $G$ and $H$ is normal in $G$, then $H$ is normal in $K$. [147]
...
$$\alpha: \{\text{subgroups } K \text{ of } G \text{ containing } H\} \to \{\text{subgroups of } G/H\} \text{ [150]}$$
defined by $\alpha(K) = K/H$ is a bijection preserving inclusions.

\textbf{Proof} \\
The group $K/H$ consists of the cosets $aH \in G/H$ with $a \in K$, and in this way it is a subset (and obviously a subgroup) of $G/H$. [152, 155] It is also true that if $H \subseteq K \subseteq L$ then $\alpha(K) = K/H \subseteq L/H = \alpha(L)$; [155] and so $\alpha$ preserves inclusions. [156] Next we verify that $\alpha$ is a bijection and to do this it suffices to write down an inverse function [157]
$$\beta: \{\text{subgroups of } G/H\} \to \{\text{subgroups } K \text{ of } G \text{ containing } H\}. \text{ [161]}$$
Let $K'$ be a subgroup of $G/H$;  define $\beta(K')$ to be the subset of $G$: [162]
$$K := \pi^{-1}(K') = \{a \in G \mid aH \in K'\}. $$
where $\pi: G \to G/H$ is the canonical projection.  Then $K$ is a subgroup of $G$ and contains $H$ since $H = \pi^{-1}(e)$ and $e \in K'$. [165, 166] Next we show that $\alpha$ and $\beta$ are inverses of each other. [167, 170, 171] Indeed
...
$$\alpha(K) = K/H = \pi(K) = \pi(\pi^{-1}(K')) = K'. \text{ [178]}$$
...
\textbf{Proof} \\
If $N$ is normal, then consider the projection $\pi: G \to G/N$. [187, 188] The subgroup $H$ is contained in $N$, which is the kernel of this homomorphism, [190] so we obtain by Theorem 4.12 an induced homomorphism $\mu: G/H \to G/N$. [191, 194]
...
$$\ker (\tilde{\pi} \circ \pi) = \{g \in G \mid \tilde{\pi}(\pi(g)) = N/H\} = \{g \in G \mid \tilde{\pi}(gH) = N/H\} = \{g \in G \mid gH \in N/H\} = \{g \in G \mid g \in N\} = N. \text{ [209, 210]}$$
...
\begin{enumerate}
    \item $HK$ is a subgroup of $G$, and $H$ is normal in $HK$; [235, 236]
    \item $H \cap K$ is normal in $K$, and [238]
